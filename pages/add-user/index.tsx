import { FormControl, FormLabel, InputGroup, InputLeftAddon, Input, FormErrorMessage, Select, Button, InputRightElement, Center, Flex, Box } from "@chakra-ui/react";
...
export default function DashboardPage() {
    ...
    const formik = useFormik({
        ...
        validationSchema: Yup.object({
            ...
            mobile: Yup.string().length(10, 'Please enter a valid mobile number').required('Required'),
        ...
        <>
            <form onSubmit={formik.handleSubmit}>
                <Box w={`min(100%, 750px)`}>
                    ...
                    <FormControl mb={4} isInvalid={formik.touched.mobile && !!formik.errors.mobile}>
                    ...
                    <FormControl mb={4} isInvalid={formik.touched.role && !!formik.errors.role}>
                        <FormLabel ps={4} htmlFor="role">Role</FormLabel>
                        <Select placeholder='Select User Role' {...formik.getFieldProps('role')}>
                    ...
                    <FormControl mb={4} isInvalid={formik.touched.username && !!formik.errors.username}>
                        <FormLabel ps={4} htmlFor="name">Username</FormLabel>
                        <Input type="text" placeholder="Username" aria-placeholder="Username" {...formik.getFieldProps('username')}></Input>
                        <FormErrorMessage>{formik.errors.username}</FormErrorMessage>
                    </FormControl>
                    <FormControl mb={4} isInvalid={formik.touched.password && !!formik.errors.password}>
                        <FormLabel ps={4} htmlFor="name">Password</FormLabel>
                        <InputGroup>
                            <Input type={show ? 'text' : 'password'} placeholder="Password" aria-placeholder="Password" {...formik.getFieldProps('password')}></Input>
