...
export default function Navigation({ isMenuOpen }: Props) {
    ...
    return (
        <Box className={`${styles.container} ${isMenuOpen ? styles.openMenu : styles.closedMenu}`}>
                            ...
                            <AccordionItem className={`${styles.navItem}`} key={`navItem-${navIdx}`} borderTopWidth={0} borderBottomWidth={1}>
                                {
                                    ({ isExpanded }) => (
                                        <>
                                            <Flex flexDir={`row`} className={`${styles.navTitle} ${isExpanded ? styles.expandedNavItem : ''} ${router.pathname.includes(navItem.path) ? styles.activeNav : ''}`} p={3} minHeight={`48px`}>
                                                            ...
                                                            {navItem.children.map((navChild, navChildIdx) => {
                                                                return (
                                                                    <ListItem key={`navChildId-${navChildIdx}`} ps={12} pb={2}>
                                                                        <Link href={`${navItem.path}${navChild.path}`} className={(router.pathname == `${navItem.path}${navChild.path}`) ? styles.activeChild : ''}>
